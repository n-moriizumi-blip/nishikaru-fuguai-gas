...
/**
 * 【診断用・手動実行・2026-08-26】`lookupByMfgNo_`(QR読み取り時に実際に呼ばれる関数そのもの)を
 * 直接実行し、返り値をそのままログ出力する。「加工者・設備№だけ空欄になる」という報告について、
 * 生データを目視するだけでなく、実際にアプリが呼ぶ関数の出力そのものを確認して原因を切り分ける。
 * 調べたい製造番号は下のmfgNoを書き換えて使う。
 */
function debugRunLookupByMfgNo() {
  var mfgNo = 'H260800002';
  var result = lookupByMfgNo_(mfgNo);
